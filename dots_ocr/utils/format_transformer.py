...
def clean_text(text: str) -> str:
    """
    Cleans text by removing markdown syntax while preserving LaTeX.
    """
    if not text:
        return ""
    ...
    for i, pattern in enumerate(latex_patterns):
        ...
        for j, m in enumerate(matches):
            key = f"__LATEX_{i}_{j}__"
            preserved[key] = m
            text = text.replace(m, key)

    # 2. ✅ 마크다운 제거 (수식은 이미 토큰으로 치환되어 안전함)
    text = re.sub(r'^\s{0,3}#{1,6}\s*', '', text)       # 제목
    text = re.sub(r'\*\*(.+?)\*\*', r'\1', text)        # **bold**
    text = re.sub(r'\*(.+?)\*', r'\1', text)            # *italic*
    # ⚠️ '_..._' 은 제거하지 않음 (LaTeX 수식 충돌 때문에 주석 처리)

    text = re.sub(r'^\s*[-*+]\s+', '', text)            # 리스트 불릿
    text = re.sub(r'>\s*', '', text)                    # 인용
    text = re.sub(r'---+', '', text)                    # 구분선
    text = re.sub(r'`{1,3}([^`]+)`{1,3}', r'\1', text)  # 코드 블록/인라인
