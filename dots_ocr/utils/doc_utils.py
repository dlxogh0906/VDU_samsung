...
## 단일 페이지 이므로 코드 단순화
# def load_images_from_pdf(pdf_file: str, dpi: int = 200) -> list[Image.Image]:
#     """
#     Load a single-page PDF as a PIL.Image list.
#     (대회 데이터는 항상 단일 페이지이므로 리스트 크기는 1)

#     Args:
#         pdf_file (str): PDF 파일 경로
#         dpi (int): 렌더링 DPI (기본 200)

#     Returns:
#         list[Image.Image]: 길이 1짜리 이미지 리스트
#     """
#     with fitz.open(pdf_file) as doc:
#         page = doc[0]  # 항상 첫 페이지만
#         img = fitz_doc_to_image(page, target_dpi=dpi)
#         return [img]
